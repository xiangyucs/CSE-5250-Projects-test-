...
the results after "gold = ". The program also prints out a list of statistics describing utilization of the CSE5250 fake vector
units. You should consider the performance of your implementation to be the value "Total Vector 
Instructions". (You can assume every CSE5250 fake vector instruction takes one cycle on the CSE5250 fake SIMD CPU.) "Vector Utilization" 
...
Submit on Canvas. Only one handin per group is required. However, please make sure that you add your partner's name to the submission. There is one place you will need to turn in the files on Canvas: `Assignment 1 (Write-Up)`.
