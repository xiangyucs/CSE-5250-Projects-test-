...
> **Python packages (prog6 plot).** prog6's `plot.py` needs `numpy`, `matplotlib`,
> and `scikit-learn` (see `requirements.txt`). On recent Debian/Ubuntu/WSL2,
> `pip install -r requirements.txt` is blocked by PEP 668 ("externally-managed
> environment"). Use a virtual environment:
> ```bash
> python3 -m venv ~/cse5250-venv          # needs python3.11-venv on Debian/Ubuntu
> ~/cse5250-venv/bin/pip install -r prog6_kmeans/requirements.txt
> ~/cse5250-venv/bin/python prog6_kmeans/plot.py
> ```
