...
__You will run code on your own multi-core computer for this assignment.__ Build and run on your own multi-core laptop. If you run into setup trouble, contact your instructor or TA.

Modern multi-core CPUs contain several physical cores, and many support __two hardware threads__ per core (Intel calls this "Hyper-Threading"). Many CPUs can also execute AVX2 vector instructions which describe
simultaneous execution of the same __eight-wide SIMD operation__ on a vector of eight single-precision 
values. For the curious, you can look up the complete specification of your own CPU. Students that want to dig deeper might enjoy [this writeup](https://en.wikichip.org/wiki/intel/microarchitectures/kaby_lake).

Note: Performance should be judged relative to the serial (1-thread) baseline on your own machine; be sure to report your core count and what machine you ran on. You will first need to install the Intel SPMD Program Compiler (ISPC) available here: <http://ispc.github.io/>. Feel free to include your findings from running code on other machines in your report as well, just be very clear what machine you were running on. 

To get started:

1. ISPC is needed to compile many of the programs used in this assignment. ISPC can be easily installed through the following steps:  

Download the binary into a local directory of your choice.  You can get ISPC compiler binaries from the ISPC [downloads page](https://ispc.github.io/downloads.html). On Linux you can use `wget` to directly download the binary from the downloads page. For example, the `wget` line below works:
...
2. Download the starter code from the course Canvas page.
...
You can view the images using the `display` command (or any image viewer on your own computer). As you can see in the images below, the
...
2.  Extend your code to use 2, 3, 4, 5, 6, 7, and 8 threads, partitioning the image
  generation work accordingly (threads should get blocks of the image). Note that a typical CPU has a limited number of physical cores (e.g., four) but each
  core may support two hyper-threads, so it can execute roughly twice that many threads interleaved on its execution contexts. Adjust the discussion to your own machine's core count.
  In your write-up, produce a graph of __speedup compared to the reference sequential (1-thread) implementation on your own machine__ as a function of the number of threads used __FOR VIEW 1__ (report your core count). Is speedup linear in the number of threads used? In your writeup hypothesize why this is (or is not) the case? (you may also wish to produce a graph for VIEW 2 to help you come up with a good answer. Hint: take a careful look at the three-thread datapoint.)
...
-  *Hint:* Use `_cse5250_cntbits` function helpful in this problem.
...
your code with `./myexp -s 3`. *Hint:* You might find `_cse5250_init_ones` helpful.
...
`CSE5250Logger.printLog()` to help you debug.
...
the results after "gold = ". The program also prints out a list of statistics describing utilization of the fake vector
units. You should consider the performance of your implementation to be the value "Total Vector 
Instructions". (You can assume every fake vector instruction takes one cycle on the fake SIMD CPU.) "Vector Utilization" 
...
utilization. You can do this by changing the `#define VECTOR_WIDTH` value in `CSE5250intrin.h`. 
...
Now that you're comfortable with SIMD execution, we'll return to parallel Mandelbrot fractal generation (like in program 1). Like Program 1, Program 3 computes a mandelbrot fractal image, but it achieves even greater speedups by utilizing both the CPU's multiple cores and the SIMD execution units within each core.
...
If you look into detailed technical material about modern CPUs, you will find there are a complicated set of rules about how many scalar and vector instructions can be run per clock.  For the purposes of this assignment, you can assume that there are about as many 8-wide vector execution units as there are scalar execution units for floating point math.   
...
2.  There is a simple way to improve the performance of
  ...
  performance that substantially exceeds the sequential (1-thread) version of the code (as guidance, over 32 times on an 8-core machine; judge relative to your own serial baseline and report your core count)!
...
1. The dataset is generated automatically on the first run of `./kmeans` (no download needed). You can also pre-build it with `make gen && ./gen_data` (default ~200k points, which is laptop-friendly; pass a number like `./gen_data 1000000` for the original larger size). Once you have the data, compile and run `kmeans` (it may take longer than usual for the program to load the data on your first try). The program will report the total runtime of the algorithm on the data.
...
For those with access to an Apple ARM-based laptop, check out the handout [here](README_aarch64.md). Produce a report of performance of the various programs on an Apple ARM-based laptop. What speedups are you observing from SIMD execution? Please do not submit any code from running on ARM based machines to Canvas.  
...
Submit on Canvas. Only one handin per group is required. However, please make sure that you add your partner's name to the submission. You will need to turn in both your write-up and your code.

For the write-up portion:
* Your writeup, in a file called `writeup.pdf`. Please make sure both group members' names and campus IDs are in the document. (if you are a group of two)
...
Please note in your write-up where to look for your extra credit. When handed in, all code must be compilable and runnable out of the box on your own multi-core computer!
