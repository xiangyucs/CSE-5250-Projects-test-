...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  // ===========================================================================
  // CSUSB (CSE5250) HINT -- where to start
  // ---------------------------------------------------------------------------
  // Your job is to parallelize EXACTLY ONE of the three functions called in the
  // loop below (computeAssignments / computeCentroids / computeCost) to get the
  // biggest speedup.
  //
  ...
  //   prog1_mandelbrot_threads spawns std::thread, same pattern).
  // Step 3 (watch for races): only write to memory your thread owns; the
  //   distance computation itself is read-only on the shared inputs.
  //
  // Target: a clear speedup over the 1-thread baseline ON YOUR OWN LAPTOP.
  // (CSUSB grading is relative speedup vs. serial on the same machine, so a
