...
# ---------------------------------------------------------------------------
# CSUSB (CSE5250) change: build on ANY laptop, zero cloud.
# The original assignment hard-coded x86 flags (-mavx) and required an SSH
# cluster. Here we pick flags per platform so the module compiles on:
#   * Windows (inside WSL2 Ubuntu, x86_64)   * Intel Mac (x86_64)
#   * Apple Silicon Mac (arm64)              * Linux (x86_64 / aarch64)
# ---------------------------------------------------------------------------
_machine = platform.machine().lower()
_system = platform.system()
_is_x86 = _machine in ("x86_64", "amd64", "x86", "i386", "i686")

_cflags = ["-O3"]
_ldflags = [ispc_path] if ispc_path else []

# -mavx is an x86-only SIMD flag; harmless to drop on ARM (Parts 1-4 do not
# require it -- it only affects the optional ISPC extra credit).
if _is_x86:
    _cflags.append("-mavx")

# OpenMP: Apple's clang needs -Xpreprocessor -fopenmp + an explicit libomp
# (install once with: brew install libomp). Linux gcc/clang use -fopenmp.
if _system == "Darwin":
    _cflags += ["-Xpreprocessor", "-fopenmp"]
    _ldflags += ["-lomp"]
    for _base in ("/opt/homebrew/opt/libomp", "/usr/local/opt/libomp"):
        if path.exists(_base):
            _cflags.append("-I" + _base + "/include")
            _ldflags.append("-L" + _base + "/lib")
else:
    _cflags.append("-fopenmp")

mr = load(name="custom_module", sources=["module.cpp"],
          extra_cflags=_cflags, extra_ldflags=_ldflags)

# Optional staff reference module. The prebuilt module_ref.so is x86_64-Linux
# only, so on Mac / ARM / native-Windows we fall back to comparing the student
# implementation against PyTorch (which is what actually decides correctness in
# testTemplate). The "reference" timing column is then just a copy of the
# student run -- correctness is unaffected.
try:
    import module_ref as ms
    HAVE_REF = True
except Exception as _e:
    print("[CSUSB] Staff reference module not available on this platform "
          "(" + _machine + "/" + _system + "); running student-only. "
          "Correctness is still checked against PyTorch.\n")
    ms = mr
    HAVE_REF = False
