...
This assignment runs CPU-only on your own laptop. There is no cloud or remote cluster required.

To get started, Download the starter code from the course Canvas page.

Run the command below to run inference using a model trained by the course staff. You will see some randomly generated Shakespeare text.
...
Your job is to further extend your matrix multiply so that it employs blocking as discussed in the course lecture slides. You will decompose the large matrices into smaller cache-sized submatrices. Your multiply will then process the smaller submatrices before evicting them from the cache. The behavior should look like the following:
...
## Extra Credit (OPTIONAL): Optimize Further
...
* Implement `fourDimRead` (required)
* Implement `fourDimWrite` (required)
...
Please submit your writeup questions in a file `writeup.pdf`. If you did the optional work, please state so at the end of your writeup as we will manually run these. In addition, please record the performance numbers we should expect for each part that you sped up using vectorization.

* Please submit the following files:
  * module.cpp
  * module.ispc (if you attempted the extra credit)
    
* Please submit your writeup in a file called `writeup.pdf`.
