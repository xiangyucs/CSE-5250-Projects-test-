...
You may have already created thread pools and task execution libraries in other systems courses.
...
**This course is ZERO-CLOUD: you will build and run this assignment entirely on your own multi-core laptop or desktop. There is no VM, no cloud instance, and no remote setup required.**

Download the starter code from the course Canvas page.

**IMPORTANT:** DO NOT modify the provided `Makefile`. Doing so may break the grading script.
...
The `-n` command-line option specifies the maximum number of threads the task system implementation can use.  In the example above, we chose `-n 16`; you should set this to match the number of execution contexts on your own machine.  The full list of tests available to run is available via command line help  (`-h` command line option).
...
> [!TIP]
> Reference solution binaries are provided for multiple platforms. Use the binary that matches your own machine: on a newer Mac with an Apple Silicon (M-series) chip use the `runtasks_ref_osx_arm` binary, on an Intel Mac use the `runtasks_ref_osx_x86` binary, and on Linux use the matching `runtasks_ref_linux*` binary.

> [!IMPORTANT]
> Your implementation is compared against the provided reference binary on YOUR OWN machine, so the comparison is fair regardless of core count. If the reference binary won't run on your machine, performance is judged as speedup relative to the Serial baseline. Report your core count.
...
For each test, full performance points will be awarded for implementations within 20% (part A) and 50% (part B) of the provided reference implementation, compared on your own machine. If the reference binary won't run on your machine, performance is judged as speedup relative to the Serial baseline; report your core count. Performance points are only awarded for implementations that return correct answers. As stated before, the _correctness_ of your implementation may also be tested using a wider set of workloads that are not provided in the starter code.

## Handin ##

Submit on Canvas.  Your submission should include both your task system code, and a writeup describing your implementation.  The expected files in the handin are:
...
The grading scripts will run the checker code provided to you in the starter code to determine performance points.  _Your code may also be run on other applications that are not provided in the starter code to further test its correctness!_

#### Writeup Handin ####

Submit a brief writeup on Canvas, addressing the following:
